...
document.addEventListener("DOMContentLoaded", function () {
  ...
  levelPaths.forEach((path, index) => {
    ...
    // Only make levels 1-5 clickable
    if (levelNumber >= 1 && levelNumber <= 5) {
      ...
      // Levels above 5 are not clickable - add a class or style to indicate
      path.style.cursor = "default";
      console.log(
        `Level ${levelNumber} is not clickable (only levels 1-5 are enabled)`
      );
    }
  ...
  function updateArrowStates(levelNumber) {
    ...
    if (downArrow) {
      // Disable down arrow if at level 1 (minimum)
    ...
    if (upArrow) {
      // Disable up arrow if at level 5 (maximum for this script)
      if (parseInt(levelNumber) >= 5) {
  ...
  function getNextLevelUp(currentLevelNum) {
    ...
    // Maximum level is 5 for this script
    if (num < 5) {
      console.log("Next level up from", num, "is:", num + 1);
      return num + 1;
    }
    console.log("No higher level available (maximum is level 5)");
    return null;
  }
  ...
  function getPreviousLevelDown(currentLevelNum) {
    ...
    console.log("No lower level available (minimum is level 1)");
    return null;
  }
...
(() => {
  ...
  async function loadAllApartments() {
    ...
    // Get unique floor numbers from the page (only floors 1-5)
    const floorElements = document.querySelectorAll('.etage--name');
    const floors = Array.from(floorElements)
      .map(el => parseInt(el.textContent.trim(), 10))
      .filter(num => !isNaN(num) && num >= 1 && num <= 5) // Only floors 1-5

// script1-26.js — combination of script.js (levels 6–26) and script1-5.js (levels 1–5).
// All levels 1 through 26 are clickable; arrows and filters support full range.
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  levelPaths.forEach((path, index) => {
    ...
    // All levels 1 through 26 (or totalLevels) are clickable (combination of script + script1-5)
  ...
  function updateArrowStates(levelNumber) {
    ...
    if (downArrow) {
      // Disable down arrow if at level 1 (minimum is 1)
...
(() => {
  ...
  async function loadAllApartments() {
    ...
    // Get unique floor numbers from the page (floors 1 through 26 - combination of script + script1-5)
    const floorElements = document.querySelectorAll('.etage--name');
    const floors = Array.from(floorElements)
      .map(el => parseInt(el.textContent.trim(), 10))
      .filter(num => !isNaN(num) && num >= 1) // All floors 1 and above
